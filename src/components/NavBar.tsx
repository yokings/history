  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-40 transition-all duration-500 ${
        scrolled
          ? 'bg-white/70 backdrop-blur-xl border-b border-gold/20 py-3 shadow-lg shadow-gold/5'
          : 'bg-transparent py-5'
      }`}
    >
          ...
          <div className="hidden md:flex items-center gap-6">
            {navItems.slice(1).map((item) => (
              ...
              >
                ...
                />
                {item.id === 'quiz' && (
                  <span className="absolute -top-2 -right-4 text-[10px] text-cinnabar bg-cinnabar/10 px-1 rounded-sm">📖</span>
                )}
              </button>
            ))}
          </div>
